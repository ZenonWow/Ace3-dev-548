...
local ACEDB_MAJOR, ACEDB_MINOR = "AceDB-3.0", 25.1
...
local function cleanProfileKeys(sv, defaultProfile, parent)
	-- Garbage collection
	...
	for  charKey, profileName  in pairs(sv.profileKeys) do
		-- Remove identity mappings (["Yourchar"] = "Yourchar") from profileKeys.
		-- Remove default mappings (["Yourchar"] = "Default") if there is no [charKey] profile to override a default.
		if  charKey == profileName
			or  profileName == defaultProfile  and  not sv.profiles[charKey]
		then
			sv.profileKeys[charKey] = nil
		end
	end
	-- Remove profileKeys if empty at the end.
	if  not next(sv.profileKeys)  then  sv.profileKeys = nil  end
end


function AceDB.FirstExistingProfile(sv, profileList, defaultProfile)
	if  typeof(sv) == 'string'  then  sv = _G[sv]  end
	local profiles =  sv  and  sv.profiles  or  {}
	for  i, profileName  in ipairs(profileList) do
		if  profiles[profileName]  then  return profileName  end
	end
	return defaultProfile
end

-- GetProfileName() is nil-safe, can be exported to addons
function AceDB.GetProfileName(sv, useDefaultProfile, parentDB)
...
local GetProfileName = AceDB.GetProfileName


-- Actual database initialization function
local function initdb(sv, defaults, useDefaultProfile, olddb, parent)
	sv.profiles = sv.profiles or {}
	
	-- Get the profileName from profileKeys or default
	-- If useDefaultProfile is provided then a newly created profile will be global (default, in contrast to character-specific).
	local profileName, defaultProfile = GetProfileName(sv, useDefaultProfile, parent)
	-- Garbage collection:  Remove identity and default mappings from sv.profileKeys
	cleanProfileKeys(sv, defaultProfile, parent)
	...
	local keyTbl= {
		...
		["realm"] = realmKey,
		["class"] = classKey,
		-- Or classKeyEnglish for readability. Changing it would lose current class specific settings in addons:  Dominos class paging, AuraFrames SpellCooldowns, ... ?
		["race"] = raceKey,
		["faction"] = factionKey,
