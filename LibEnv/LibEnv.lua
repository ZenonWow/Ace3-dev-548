...
local LibEnv = LibStub:NewLibrary("LibEnv", 1)
if  LibEnv  then
	-- Import
	local _G = getfenv(1)
	local assert,setfenv,rawget = assert,setfenv,rawget
	local softassert = _G.softassert  or  function(pass, failMessage)  if not pass then  _G.geterrorhandler()(failMessage)  end end
	...
	function CheckVar(_ADDON, var, value)
		...
		else  softassert(was == value, '_ADDON.'..var..' = "'..tostring(was)..'" must be the same as currently provided '..var..' = "'..tostring(value)..'".')
		end
	end
	...
	NoGlobalsMeta = {
		NewEnv = function(self, env)
			lib.envData[env] = lib.envData[env] or {}
			return setmetatable(env, self)
		end,
		__index = function(env, var)
			local envData = lib.envData[env]
